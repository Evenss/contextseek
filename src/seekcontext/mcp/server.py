"""MCP-compatible server facade for SeekContext tools."""
...
from seekcontext.domain.serialization import serialize_context_item


@dataclass
class SeekContextMCPServer:
    """MCP tool server that exposes SeekContext operations as tool calls."""
    ...
    def list_tools(self) -> list[dict[str, Any]]:
        ...
        return [
            {
                "name": "seekcontext_add",
                "description": "Add content to SeekContext",
                "parameters": {
            ...
            {
                "name": "seekcontext_retrieve",
                "description": (
                    "Retrieve from SeekContext: returns ranked SearchHits with L1 "
            ...
            {
                "name": "seekcontext_evidence_chain",
            ...
            {
                "name": "seekcontext_chain_confidence",
            ...
            {
                "name": "seekcontext_skill_context",
    ...
    def call_tool(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "seekcontext_evidence_chain":
            scope = arguments["scope"]
            item_id = arguments["item_id"]
            ref = (
                item_id
        ...
        if name == "seekcontext_chain_confidence":
            scope = arguments["scope"]
            item_id = arguments["item_id"]
            ref = (
                item_id
        ...
        if name == "seekcontext_skill_tools":
            scope = arguments["scope"]
        ...
        if name == "seekcontext_skill_context":
            scope = arguments["scope"]
            query = arguments.get("query") or None
