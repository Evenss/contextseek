"""SeekContext — the unified client for semantic context management.

This is the primary API surface for SeekContext: one flat class with
...
from seekcontext.domain.conflicts import ConflictType
...
from seekcontext.domain.tools import EXPAND_HINT, ToolSpec, default_tool_specs
from seekcontext.llm.prompts import (
    LLMPromptTemplates,
...
from seekcontext.llm.client import invoke_json, invoke_text
from seekcontext.llm.parsers import extract_json_object
...
# - seekcontext.evolution.engine.EvolutionEngine
# - seekcontext.observability.audit.AuditLog

# ---------------------------------------------------------------------------
# Context variable for audit metadata (request-scoped)
# ---------------------------------------------------------------------------
_AUDIT_CONTEXT: ContextVar[dict[str, Any]] = ContextVar(
    "seekcontext_audit_ctx", default={}
)
...
def _make_default_adapter() -> SeekVFSAdapter:
    ...
    vfs = VFS(
        routes={"seekcontext://": {"backend": InMemoryBackend()}},
        scheme="seekcontext://",
    )
...
def _auto_build_summarizer() -> Any | None:
    ...
    """
    from seekcontext.config.factory import build_summarizer
    from seekcontext.config.settings import SummarizerSettings
...
# ---------------------------------------------------------------------------
# SeekContext dataclass
# ---------------------------------------------------------------------------


@dataclass
class SeekContext:
    """Unified SeekContext client — all operations on ContextItems.

    The SeekContext client wraps a storage adapter and exposes a clean API
    ...
    def add(
        ...
        *,
        ...
        if check_conflicts:
            from seekcontext.domain.conflicts import ConflictType, detect_conflicts
    ...
    def retrieve(
        ...
        *,
        ...
        from seekcontext.retrieval.components import LLMReranker
    ...
    def chain_confidence(self, ref: str, *, scope: str) -> float:
        ...
        """
        from seekcontext.domain.evidence_chain import compute_chain_confidence
    ...
    def skill_tools(
        ...
        *,
        ...
        """
        from seekcontext.domain.skill_executor import SkillExporter
    ...
    def execute_skill(
        ...
        *,
        ...
        """Deprecated — SeekContext no longer executes skills.
        ...
        warnings.warn(
            "execute_skill() is deprecated. SeekContext no longer executes skills. "
    ...
    def dream(
        ...
        *,
        ...
        from seekcontext.evolution.dreaming import DreamEngine
    ...
    def pin(self, version: str) -> "SeekContext":
        ...
        Returns:
            New ``SeekContext`` with the given version label.
        """
        new_instance = replace(self, _strategy_version=version)
        return new_instance

    @classmethod
    def from_runtime_config(cls, path: str | None = None) -> "SeekContext":
        """Factory: create a SeekContext from a runtime config file.
        ...
        Returns:
            Configured SeekContext instance.
        """
        if path is None:
            return cls()
        ...
        if adapter_config:
            ...
            if adapter_type == "in_memory":
                ...
                backend = FileBackend(root=adapter_config.get("root", ".seekcontext"))
                vfs = VFS(
        ...
        # Build resolver
        uri_scheme = config.get("uri_scheme", "seekcontext://")
    ...
    def from_settings(
        ...
        """Factory: create a SeekContext from pydantic-settings configuration.

        Loads adapter, resolver, embedder, evolution engine, and audit log
        from a SeekContextSettings instance.  If no settings object is passed,
        one is created automatically (reading from environment variables and
        .env file).

        Args:
            settings: Optional SeekContextSettings instance.  Created from
                environment if None.

        Returns:
            Fully configured SeekContext instance.
        """
    ...
    def _emit_audit(
        ...
        *,
        ...
        from seekcontext.observability.audit import AuditRecord
    ...
    def _retrieval_strategy(self):
        ...
        from seekcontext.config import RetrievalStrategy
    ...
    def _propagate_invalidation(self, deleted_item: ContextItem, scope: str):
        ...
        from seekcontext.domain.invalidation import InvalidationResult, propagate_invalidation
