"""Deep Agents context store adapter backed by SeekContext (unified ContextItem API)."""
...
from seekcontext.bridges.compat import DEEPAGENTS_AVAILABLE

if TYPE_CHECKING:
    from seekcontext.client.seekcontext import SeekContext
    from seekcontext.domain import ContextItem, RetrieveResponse
...
class ContextStore:
    ...
    def spec(cls) -> AdapterSpec:
        return AdapterSpec(
            name="seekcontext.deepagents.context_store",
    ...
    @classmethod
    def from_client(cls, client: "SeekContext", *, scope: str) -> "ContextStore":
