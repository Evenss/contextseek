"""SeekContext × LangChain Agent Middleware.

将 SeekContext 的检索/存储/演化能力接入 LangChain ``create_agent()`` 流水线。
...
if TYPE_CHECKING:
    from seekcontext.config.settings import EmbeddingSettings
...
_SCOPE_VAR: ContextVar[str | None] = ContextVar(
    "seekcontext_middleware_scope", default=None
)


class SeekContextMiddleware(AgentMiddleware[AgentState[ResponseT], ContextT, ResponseT]):
    """LangChain ``AgentMiddleware`` that bridges SeekContext into the agent loop.

    - ``wrap_model_call``: retrieve relevant context and append it to ``system_message``
    - ``after_model``: persist the latest assistant turn to SeekContext
    ...
    def __init__(
        ...
        *,
        ...
        self._compact_executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="seekcontext-compact"
        )
        ...
        if ctx is not None:
            ...
            self.ctx = self._build_seekcontext(model, embedder)

    # ── construction ──────────────────────────────────────

    def _build_seekcontext(
        self,
