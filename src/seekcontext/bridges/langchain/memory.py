"""LangChain memory adapter for SeekContext (unified ContextItem API)."""
...
from seekcontext.bridges.compat import AIMessage
from seekcontext.bridges.compat import BaseChatMessageHistory
from seekcontext.bridges.compat import BaseMessage
from seekcontext.bridges.compat import ConfigDict
from seekcontext.bridges.compat import HumanMessage
from seekcontext.bridges.compat import LANGCHAIN_MEMORY_AVAILABLE
from seekcontext.bridges.compat import SystemMessage

if TYPE_CHECKING:
    from seekcontext.client.seekcontext import SeekContext
    from seekcontext.domain import ContextItem
...
def _langchain_stored_items(client: Any, scope: str, k: int) -> list[Any]:
    ...
    Uses :meth:`SeekContext.items` (full enumeration) instead of ``retrieve`` so
...
_SPEC = AdapterSpec(
    name="seekcontext.langchain.memory",
    framework="langchain",
    capabilities=(AdapterCapability.MEMORY,),
    description="LangChain chat history adapter backed by SeekContext unified ContextItem API.",
...
if LANGCHAIN_MEMORY_AVAILABLE:
    ...
    @dataclass
    class SeekContextMemory(BaseChatMessageHistory):
        """Message history adapter aligned with `langchain_core` contracts.

        Uses the unified SeekContext client with add()/retrieve() API.
        """
        ...
        def add_messages(self, messages: list[BaseMessage]) -> None:
            """Append LangChain messages into SeekContext via add()."""
        ...
        @classmethod
        def from_client(
            cls, client: "SeekContext", *, scope: str, k: int = 10
        ) -> "SeekContextMemory":
    ...
    @dataclass
    class SeekContextMemory(BaseChatMessageHistory):  # type: ignore[no-redef]
