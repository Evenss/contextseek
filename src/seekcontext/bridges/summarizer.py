...
The API layer (``SeekContext``) is the only place that calls a Summarizer.
:class:`LLMSummarizer` wraps any LangChain ``BaseChatModel`` to produce
controlled-length summaries.

When no LLM is available SeekContext falls back to flat L2-only mode
...
class LLMSummarizer:
    ...
    :func:`seekcontext.config.factory.build_llm`) and injected. Both prompts
