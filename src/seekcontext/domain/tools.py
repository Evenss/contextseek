...
tools format. The list returned by `SeekContext.tools()` shares the same
...
_RETRIEVE_SPEC = ToolSpec(
    name="retrieve",
    description=(
        "Search the SeekContext store and return ranked hits as summaries (L1). "
...
def default_tool_specs() -> list[ToolSpec]:
    """Return SeekContext's built-in tool specs (retrieve + expand)."""
