"""Minimal FastAPI server for SeekContext SDK."""
...
try:
    ...
    msg = (
        ...
        "Install with: pip install seekcontext[http]"
    )
...
def create_app(client: SeekContext | None = None) -> FastAPI:
    """Create FastAPI application backed by SeekContext."""
    app = FastAPI(title="SeekContext API", version=PACKAGE_VERSION)
    ctx = client or SeekContext.from_settings()
