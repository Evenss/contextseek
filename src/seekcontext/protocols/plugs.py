"""DataPlug protocol — streaming ingestion adapters for SeekContext.

A DataPlug is any source that can stream structured events into the
SeekContext graph.  Implementations wrap specific data sources (git logs,
Slack channels, document crawlers, etc.) behind a uniform iterator
interface so that `SeekContext.plug()` can consume them generically.
"""
...
class RawEvent:
    ...
    and minimal metadata; the SeekContext client is responsible for
...
class DataPlug(Protocol):
    ...
    def stream(self) -> Iterator[RawEvent]:
        ...
        the SeekContext client. If the source is unbounded, the plug should
