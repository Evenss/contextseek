"""Local CLI for SeekContext business-level operations."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(prog="seekcontext")
    ...
    tools_parser = subparsers.add_parser(
        "tools",
        help="print SeekContext LLM tool spec (OpenAI/Anthropic format)",
    )
...
def run_cli(
    argv: Sequence[str] | None = None, *, client: SeekContext | None = None
) -> int:
...
def main() -> int:
    """Entry point used by `python -m seekcontext.cli.main`."""
    return run_cli()
