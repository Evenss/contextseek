...
class AuditLog:
    ...
    def export_prometheus(self) -> str:
        ...
        lines = [
            "# HELP seekcontext_request_metric SeekContext request-level metric.",
            "# TYPE seekcontext_request_metric gauge",
        ]
        for record in self.records:
            base_labels = {
            ...
            lines.append(f"seekcontext_request_metric{{{labels}}} {record.elapsed_ms}")
            for metric in record.metrics:
                labels = _labels({**base_labels, **metric.tags, "name": metric.name})
                lines.append(f"seekcontext_request_metric{{{labels}}} {metric.value}")
        return "\n".join(lines) + "\n"
