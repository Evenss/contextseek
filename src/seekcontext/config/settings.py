"""Unified pydantic-settings configuration for SeekContext.
...
from seekcontext.llm.prompts import DEFAULT_LLM_PROMPTS
...
def settings_config(
    *,
    ...
    """Options for the root ``SeekContextSettings`` (PowerMem-style .env + case folding)."""
...
class StorageSettings(BaseSettings):
    ...
    path: str = ".seekcontext/store"
    """Root path for file-based backend."""

    uri_scheme: str = "seekcontext://"
    ...
    cold_path: str = ".seekcontext/cold"
...
class OceanBaseSettings(BaseSettings):
    ...
    password: str = ""
    db_name: str = "test"
    table_name: str = "seekcontext_items"
...
class SummarizerSettings(BaseSettings):
    """Summarizer configuration — drives L0/L1 generation in ``SeekContext.add()``.
    ...
    ``None`` and SeekContext falls back to flat L2-only behavior.
    """
...
class ObservabilitySettings(BaseSettings):
    ...
    audit_path: str = ".seekcontext/audit.jsonl"
    metrics_enabled: bool = False
    metrics_path: str = ".seekcontext/metrics.prom"
...
class SeekContextSettings(BaseSettings):
    """Unified configuration for SeekContext.
...
def to_strategy_config(settings: SeekContextSettings) -> "StrategyConfig":
    """Convert SeekContextSettings to the legacy StrategyConfig dataclass.
