"""Lazy model factory for SeekContext.
...
from seekcontext.config.settings import EmbeddingSettings, LLMSettings, SummarizerSettings
...
def build_embedder(settings: EmbeddingSettings) -> Callable[[str], list[float]] | None:
    ...
    import seekcontext.embedders.langchain_embedder as _lc_mod
...
def build_summarizer(
    ...
    *,
    ...
    Returns:
        ...
        :class:`~seekcontext.bridges.summarizer.LLMSummarizer` when
    ...
    if settings.provider == "llm":
        from seekcontext.bridges.summarizer import LLMSummarizer
