...
SeekContext 从**环境变量**和可选的 **`.env` 文件**加载配置。零配置路径无需任何设置：内存存储 + 关键词召回。
...
在 `SeekContextSettings(...)` 里显式赋值的字段优先于环境变量。
...
无需配置，`SeekContext.from_settings()` 即可。
...
安装：`pip install "seekcontext[langchain,openai]"`。
...
| `STORAGE_PATH` | `.seekcontext/store` | `file` 时目录 |
| `STORAGE_URI_SCHEME` | `seekcontext://` | ref 协议 |
| `STORAGE_COLD_BACKEND` | 空 | 可选冷层 |
| `STORAGE_COLD_PATH` | `.seekcontext/cold` | 冷层路径 |
...
`OPENAI_API_KEY` 等由 LangChain 类读取，非 SeekContext 直接解析。
...
| `OBSERVABILITY_AUDIT_PATH` | `.seekcontext/audit.jsonl` | 路径 |
