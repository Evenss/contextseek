...
SeekContext 是面向 Agent 的**上下文资产层**：统一写入（`add`）、统一排名检索（`retrieve`），并可选地将原始观测演进为可复用技能。
...
无明确来源、永不被搜索、或纯临时缓冲的数据，应放在 SeekContext 之外（如 Redis 会话缓存、原始日志文件）。
...
许多系统将 profile、会话、知识库、Trace、技能分成不同表和 API。SeekContext 统一为一种对象，因为：
