...
SeekContext 提供两个可观测性数据流：**审计日志**（JSONL 格式，每次 API 调用一条记录）和 **Prometheus 兼容指标**（导出为文本文件）。两者默认均关闭。
...
SeekContext 在每次被审计的调用后将 Prometheus 文本写入 `metrics_path` 文件。可用 Prometheus 文件 exporter 采集，或从自定义 HTTP 端点提供。
