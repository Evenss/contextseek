"""Adapter wrapping the SeekContext-enabled AppWorld ReAct agent."""
...
def build_scope(config: dict[str, Any]) -> str:
    """Render the SeekContext scope from config and experiment metadata."""
    ...
    return template.format(
        ...
        agent_type=config.get("type", "seekcontext_react"),
    )


class SeekContextReactAdapter(AgentAdapter):
    """Run AppWorld tasks with SeekContext retrieval and trajectory writes."""

    @property
    def name(self) -> str:
        return "seekcontext_react"
    ...
    def configure(self, config: dict[str, Any]) -> None:
        ...
        if seek_cfg.get("enabled", True):
            seekcontext_client = SeekContextClient.from_config(
                seek_cfg,
        ...
        self._agent = AppWorldSeekContextAgent(
            ...
            seekcontext_client=seekcontext_client,
    ...
    def run_task(self, task_id: str, **kwargs: Any) -> RunResult:
        if self._agent is None:
            raise RuntimeError("SeekContextReactAdapter.configure() must be called first")
