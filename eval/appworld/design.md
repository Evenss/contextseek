# SeekContext AppWorld 评测方案

## 0. 为什么需要 AppWorld 评测

SeekContext 的价值在于让 agent 在多次任务中积累经验、记住约束、从错误中学习。评估这个能力需要多样化的评测环境，AppWorld 在评测体系中扮演**大规模 API 多样性验证**的角色：
...
与 τ-bench 不同，AppWorld 的特点是**海量 API（457 个，覆盖 9 个 app）**，每个任务可能调用多个 app 的接口。这验证的是 SeekContext 在"大海捞针"场景下的检索精度和多源信息的组织能力，这和seekcontext的“插座” 属性契合。

---

## 1. 评测目标

本评测不是要造一个新的 AppWorld agent，而是在**同一个 ReAct Agent 框架**下，对比不同 SeekContext 接入方式对任务效果的影响：

- **Baseline** — 不接入 SeekContext，纯 ReAct agent
- **File backend** — SeekContext 本地文件存储，验证最基本的上下文写入、蒸馏、召回链路
- **OceanBase backend** — 向量 + 全文混合检索，对照 file backend 验证检索质量提升
- **Evolve** — 开启 SeekContext 压缩演进能力，观察自动沉淀后的效果变化
...
run.py (CLI 入口)
  │
  ├── run stage      →  执行任务，收集轨迹
  ├── distill stage  →  从轨迹提取经验，写入 SeekContext
...
- **evaluate stage** — 汇总成功率、步数、token 消耗、SeekContext 检索命中率

### 2.2 双环境隔离架构

这是 AppWorld 评测与 τ-bench 评测最大的架构差异。AppWorld 依赖 pydantic 1.x + SQLAlchemy 1.4，而 SeekContext 依赖 pydantic 2.x + SQLAlchemy 2.x，二者无法共存于同一 Python 环境：

```
┌─── .venv (主进程) ──────────────────────────┐
│  run.py                                      │
│  AppWorldSeekContextAgent (ReAct 循环)        │
│  LLMClient (OpenAI-compatible / Azure)        │
│  SeekContextClient (检索/写入/反馈/compact)    │
│  File Backend (.seekcontext/appworld)         │
...
### 2.3 SeekContext 在 ReAct 循环中的注入时机

SeekContext 在三个关键时机介入：

**时机 1 — 任务开始时**。获取 AppWorld 任务指令后，先从 SeekContext 检索相关背景（同 app 的历史经验、相似错误的处理方式），将检索结果插入 system prompt。这相当于给 agent 一份"赛前简报"。
...
**时机 3 — 任务结束时**。轨迹存入 SeekContext，供后续任务检索。若成功且使用了检索到的经验，对该经验施加正向反馈。若开启 auto_compact，触发压缩演进。
...
├── context.py                 # SeekContext 客户端（支持 file/OceanBase 双后端）
...
│   ├── baseline.py            # 基线（无 SeekContext）
│   ├── seekcontext_react.py   # SeekContext 增强（检索 + 存储 + 反馈 + compact）
│   └── official_simplified.py # 官方 simplified adapter
├── config/
│   ├── baseline.yaml
│   ├── seekcontext_store_only.yaml
│   ├── seekcontext_react.yaml
│   ├── seekcontext_evolve.yaml
│   ├── seekcontext_store_only_oceanbase.yaml
│   └── seekcontext_react_oceanbase.yaml
└── pipeline/
...
这纯粹是依赖冲突驱动的。AppWorld 构建于 pydantic 1.x 时代，和 SeekContext 的 pydantic 2.x 无法共存。因此采用子进程 + JSONL 协议方案，这是最小化侵入性的隔离方式。
...
这也带来了好处——循环的每一步都是可控的，可以在任意时机插入 SeekContext 检索（比如出错时）。
...
File backend 是简单的本地文件存储，适合快速验证。但 SeekContext 的长期目标是支撑大规模、多轮次的经验积累，因此需要验证向量 + 全文混合检索在大数据量下的表现。OceanBase 提供了这种能力，同时作为 file backend 的对照，帮助判断是否需要引入更复杂的存储后端。
...
| 实验组 | 目的 | 预期提供的结论 |
|--------|------|---------------|
| **Baseline** | 纯 ReAct agent，无 SeekContext | 基线成功率 / 步数 / token 消耗 |
| **File Store Only** | 预热，只写不读 | 构建 SeekContext 知识库雏形 |
...
| 指标 | 含义 | 关注点 |
|------|------|--------|
| **Success Rate** | 任务通过比例 | 核心指标，SeekContext 应显著高于 Baseline |
| **Avg Steps** | 平均 ReAct 步数 | 步数降低说明经验帮助 agent 更快找到正确路径 |
| **Token 消耗** | prompt + completion tokens | 上下文注入带来的 token 增长 vs 减少步数省下的 token |
| **Error Recovery Rate** | 出错后最终成功的比例 | SeekContext 错误检索的价值最直接的体现 |

### 5.2 SeekContext 专有指标
...
- `context_helped` — Baseline 失败但 SeekContext 成功 → 证明检索经验有价值
- `context_hurt` — Baseline 成功但 SeekContext 失败 → 检索噪声干扰了 agent
...
- 缺少 per-app 成功率统计，无法判断 SeekContext 在不同 app 上的表现差异
